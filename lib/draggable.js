...
module.exports = React.createClass({
	...
	propTypes: {
		 ...
		 *
		 * The property takes a list of space-separated strings.
		 *
		 ...
		 * 'all' bounds movement to all edges.
		 *
		 * Optionally choose one anchor of:
		 * 'point' to constrain only the top-left corner.
		 * 'box' to constrain the entire box.
		 *
		 * You may use more than one bound, e.g. 'top left point'. If no anchor
		 * is given, the default is 'box'.
		 *
		 * Defaults to null.
		 */
	...
	getInitialState: function () {
		...
		return {
			...
			// Parent bounds relative to DOMNode
			...
			// Bounding options
			boundTop: boundAll || !!~bound.indexOf('top'),
			boundRight: boundAll || !!~bound.indexOf('right'),
			boundBottom: boundAll || !!~bound.indexOf('bottom'),
			boundLeft: boundAll || !!~bound.indexOf('left'),
			boundBox: !~bound.indexOf('point')
		};
	},
	...
	componentDidMount: function () {
		...
		var parentLeft = this.state.parentLeft;
		var parentTop = this.state.parentTop;
		var nodeBound, parentNode, parentBound;

		// Assume CSS position is relative
		nodeBound = node.getBoundingClientRect();
		parentNode = node.parentNode;
		parentBound = parentNode.getBoundingClientRect();
		parentLeft -= nodeBound.left - parentBound.left - parentNode.clientLeft;
		parentTop -= nodeBound.top - parentBound.top - parentNode.clientTop;

		// Get pixel values for the boundaries offset
		this.setState({
	...
	componentWillReceiveProps: function (nextProps) {
		...
		if (nextProps.start) {
			if (nextProps.start.x != null) {
				state.offsetLeft = nextProps.start.x;
			}
			if (nextProps.start.y != null) {
				state.offsetTop = nextProps.start.y;
			}
			this.setState(state);
		}
	},
	...
	handleDrag: function (e) {
		...
		if (canDragX(this)) {
			...
			boundingValue = this.state.parentLeft;
			if (this.state.boundLeft && offset < boundingValue) {
				offset = boundingValue;
			}
			if (this.state.boundRight) {
				boundingValue += node.parentNode.clientWidth;
		...
		if (canDragY(this)) {
			...
			// Bound movement to parent box
			boundingValue = this.state.parentTop;
			if (this.state.boundTop && offset < boundingValue) {
				offset = boundingValue;
			}
			if (this.state.boundBottom) {
				boundingValue += node.parentNode.clientHeight;
				if (this.state.boundBox) {
		...
		// Save new state
		state.clientX = this.state.clientX +
			state.offsetLeft - this.state.offsetLeft;
		state.clientY = this.state.clientY +
			state.offsetTop - this.state.offsetTop;
		this.setState(state);
