...
function updateOffset (state, node) {
	var style = window.getComputedStyle(node);
	var left = state.offsetLeft || style.getPropertyValue('left');
	var top = state.offsetTop || style.getPropertyValue('top');

	// Support percentages
	if (typeof left == 'string' && left.slice(-1) == '%') {
		left = (parseInt(left, 10) / 100) * node.offsetParent.clientWidth;
	} else {
		left = parseInt(left);
	}
	if (typeof top == 'string' && top.slice(-1) == '%') {
		top = (parseInt(top, 10) / 100) * node.offsetParent.clientHeight;
	} else {
		top = parseInt(top);
	}
...
module.exports = React.createClass({
	...
	componentWillReceiveProps: function (nextProps) {
		...
		updateOffset(state, this.getDOMNode());
		this.setState(state);
	},
