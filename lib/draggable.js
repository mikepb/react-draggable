...
module.exports = React.createClass({
	...
	handleDragStart: function (e) {
		...
		// Support percent offset
		var startX = parseInt(node.style.left, 10) || 0;
		var startY = parseInt(node.style.top, 10) || 0;
		if (String(node.style.left).slice(-1) == '%') {
			startX = node.offsetParent.clientWidth * (startX / 100);
		}
		if (String(node.style.top).slice(-1) == '%') {
			startY = node.offsetParent.clientHeight * (startY / 100);
		}
